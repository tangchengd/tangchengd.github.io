...
class Topic {
  ...
  constructor(element, level) {
    ...
    this.name =
      element.textContent.length >= MAX_TOC_LEN
        ? element.textContent.substring(0, MAX_TOC_LEN) + "..."
        : element.textContent;
...
const TocComponent = Vue.defineComponent({
  ...
  setup() {
    ...
    const adjustTocLabelPos = () => {
      ...
      let rate =
        ((1 - toc.value.clientHeight / document.documentElement.clientHeight) *
          100) /
        2;
      // TODO 解决溢出的问题
